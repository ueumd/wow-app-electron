...
export class NodeFseApi {
	 ...
	 * @Return Boolean
	 */
	pathExistsSyncAndFileStatus(filePath): boolean {
		const exists = fse.pathExistsSync(filePath)
		if (exists) {
			// 检测文件是否有效
			const status = fse.statSync(filePath)
			if (status.size === 0) {
				return false
			}
		}
	 ...
	 * @Returen Promise
	 */
	downloadFile({ url, filePath, axiosConfig }: { url: string; filePath: string; axiosConfig: AxiosRequestConfig }) {
		return new Promise((resolve, reject) => {
			...
			return axios({
				...
				resp => {
					return this.createFile(resp.data, fullPath)
				},
				err => {
					console.log(err)
				}
